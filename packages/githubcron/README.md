...
```typescript
import { ServerlessCron } from "npm:@snowinch/githubcron";
...
cron.job("daily-digest", {
  ...
  handler: async () => {
    ...
    const sent = await Promise.all(users.map((user) => sendDigestEmail(user)));
...
- `@snowinch/html-to-pdf` - HTML to PDF conversion
- More coming soon!
